...
def attempt_play_click(driver):
    """Attempt to find, scroll to, and click play buttons using generic CSS selectors via JavaScript injection."""
    print(f"{now()} Attempting to find, scroll to, and click play buttons...")
    js_code = """
    var selectors = [
        '.vjs-big-play-button',    // Video.js large button (matches your screenshot style)
        'button[aria-label*="play" i]',
        'button[title*="play" i]',
        '.vjs-play-control',       // Video.js control bar
        ...
        '.amp-default-play',       // Akamai Media Player
        '#play-button',
        '.play-button',
        '.play-btn',
        '.play',
        '#my-video',               // Specific to this 123movies template
        'video'                    // Clicking the video element directly
    ];
    var clicked = 0;
    
    // 1. Try to find standard play buttons and click them
    for (var i = 0; i < selectors.length; i++) {
        var els = document.querySelectorAll(selectors[i]);
        for (var j = 0; j < els.length; j++) {
            if (els[j].offsetParent !== null) { // Ensure element is somewhat visible in the DOM
                els[j].scrollIntoView({behavior: 'smooth', block: 'center', inline: 'center'});
                els[j].click();
                clicked++;
            }
        }
    ...
    // 2. Fallback: If no standard button was found, find the iframe/video container, scroll to it, and click it
    if (clicked === 0) {
        var containers = document.querySelectorAll('iframe, .video-container, .player-container, figure');
    ...
    """
    try:
        clicked_count = driver.execute_script(js_code)
        if clicked_count > 0:
            print(f"{now()} Found, scrolled to, and clicked {clicked_count} potential play element(s) via JS.")
        else:
            print(f"{now()} Warning: No video containers or play buttons found to click.")
    except Exception as e:
        print(f"{now()} Warning: Error while attempting to click play: {e}")
...
def main():
    ...
    try:
        ...
        # small initial sleep to let players init 
        time.sleep(2.0)
